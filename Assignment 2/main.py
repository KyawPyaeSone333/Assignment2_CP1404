...
Name:Kyaw Pyae Sone
Date:23.09.2019
Brief Project Description: Travel Tracker
GitHub URL:
"""
...
#initial function to build up the UI
    def __init__(self):
        super().__init__()
...
#function to load the KV file
    def build(self):
        self.title = "TravelTracker 2.0 by Kyaw Pyae Sone"
        self.root = Builder.load_file('app.kv')
        self.places_list.load_places()
        self.build_widgets_left_col()
        self.build_widgets_right_col()
        return self.root
#padding buttons on left widget
...
#function to clear the field after one input
        ...
        for place in self.places_list.places_list:
            if place[0].status == "v":
                song_display_button = Button(
                    ...
                    id=place[0].place)
                song_display_button.background_color = [120, 30, 88, 0.1]
            else:
                song_display_button = Button(
                    text="'{}' by  {} ({})".format(place[0].place, place[0].country, place[0].priority), id=place[0].place)
                song_display_button.background_color = [120, 50, 0, 0.5]
